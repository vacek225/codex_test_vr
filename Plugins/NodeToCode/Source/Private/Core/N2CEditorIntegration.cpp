...
void FN2CEditorIntegration::ExecuteCreateLocalHandoffForEditor(TWeakPtr<FBlueprintEditor> InEditor)
{
    FN2CLogger::Get().Log(TEXT("ExecuteCreateLocalHandoffForEditor called"), EN2CLogSeverity::Debug);
    ...
    FString GraphName = FocusedGraph->GetName();
    FString BlueprintName = TEXT("Unknown");
    if (UBlueprint* BlueprintObject = Editor->GetBlueprintObj())
    {
        BlueprintName = BlueprintObject->GetName();
    }
    ...
    if (!Collector.CollectNodesFromGraph(FocusedGraph, CollectedNodes))
    {
        FN2CLogger::Get().LogError(TEXT("Failed to collect nodes for local handoff"));
        return;
    }
    ...
    if (!Translator.GenerateN2CStruct(CollectedNodes))
    {
        FN2CLogger::Get().LogError(TEXT("Failed to translate nodes for local handoff"));
        return;
    }
    ...
    if (!Blueprint.IsValid())
    {
        FN2CLogger::Get().LogError(TEXT("Generated Blueprint data is invalid for local handoff"));
        return;
    }
    ...
    if (!LLMModule)
    {
        FN2CLogger::Get().LogError(TEXT("Failed to get LLM module for local handoff"));
        return;
    }

    if (!LLMModule->SaveLocalHandoffToDisk(Blueprint))
    {
        FNotificationInfo Info(NSLOCTEXT("NodeToCode", "LocalHandoffFailed", "Failed to create local Codex handoff"));
        Info.bFireAndForget = true;
        Info.FadeInDuration = 0.2f;
        Info.FadeOutDuration = 0.5f;
    ...
    const FString HandoffPath = LLMModule->GetLatestTranslationPath();
    if (!HandoffPath.IsEmpty())
    {
        FPlatformApplicationMisc::ClipboardCopy(*HandoffPath);
    }

    FN2CGraphTranslation SummaryGraph;
    SummaryGraph.GraphName = GraphName;
    SummaryGraph.GraphType = TEXT("LocalCodexHandoff");
    SummaryGraph.GraphClass = Blueprint.Metadata.BlueprintClass;
    SummaryGraph.Code.GraphImplementation = FString::Printf(
        TEXT("// Local Codex handoff created successfully\n")
        TEXT("// Blueprint: %s\n")
        TEXT("// Graph: %s\n")
        TEXT("// Saved folder: %s\n")
        TEXT("// The folder path has been copied to the clipboard.\n")
        TEXT("// Next step: ask Codex to read the latest handoff and generate UE 5.7 code.\n"),
        *BlueprintName,
        *GraphName,
        *HandoffPath);
    SummaryGraph.Code.ImplementationNotes = TEXT("This result was created locally. No cloud provider request was made.");

    FN2CTranslationResponse LocalResponse;
    LocalResponse.Graphs.Add(SummaryGraph);
    LLMModule->OnTranslationResponseReceived.Broadcast(LocalResponse, true);

    FNotificationInfo Info(NSLOCTEXT("NodeToCode", "LocalHandoffCreated", "Local Codex handoff created and copied to clipboard"));
    Info.bFireAndForget = true;
    Info.FadeInDuration = 0.2f;
    Info.FadeOutDuration = 0.5f;
    ...
    FN2CLogger::Get().Log(
        FString::Printf(TEXT("Local Codex handoff created for Blueprint: %s at %s"), *BlueprintName, *HandoffPath),
        EN2CLogSeverity::Info,
        TEXT("EditorIntegration"));
}
...
void FN2CEditorIntegration::RegisterToolbarForEditor(TSharedPtr<FBlueprintEditor> InEditor)
{
    ...
    // Map the Local Codex Handoff command
    CommandList->MapAction(
        FN2CToolbarCommand::Get().CreateCodexHandoffCommand,
        FExecuteAction::CreateLambda([this, WeakEditor, BlueprintName]()
        {
            FN2CLogger::Get().Log(
                FString::Printf(TEXT("Create Local Codex Handoff triggered for Blueprint: %s"), *BlueprintName),
                EN2CLogSeverity::Info
            );
            ExecuteCreateLocalHandoffForEditor(WeakEditor);
        }),
        FCanExecuteAction::CreateLambda([WeakEditor]()
        {
            TSharedPtr<FBlueprintEditor> Editor = WeakEditor.Pin();
        ...
        FToolBarExtensionDelegate::CreateLambda([CommandList](FToolBarBuilder& Builder)
        {
            ...
            Builder.AddComboButton(
                ...
                FOnGetContent::CreateLambda([CommandList]() -> TSharedRef<SWidget>
                {
                    ...
                    MenuBuilder.AddMenuEntry(FN2CToolbarCommand::Get().CreateCodexHandoffCommand);
...
void FN2CEditorIntegration::ExecuteCollectNodesForEditor(TWeakPtr<FBlueprintEditor> InEditor)
{
    ...
    UN2CLLMModule* LLMModule = UN2CLLMModule::Get();
    if (LLMModule && LLMModule->IsTranslationRequestInFlight())
    {
