...
public:
    ...
    EN2CSystemStatus GetSystemStatus() const;

    /** Check if an LLM translation request is currently in flight */
    bool IsTranslationRequestInFlight() const { return bTranslationRequestInFlight; }
    ...
    /** Save a local handoff package for Codex without calling an LLM provider */
    bool SaveLocalHandoffToDisk(const FN2CBlueprint& Blueprint);

private:
    /** Generate file paths for translation */
    FString GenerateTranslationRootPath(const FString& BlueprintName) const;

    /** Generate file paths for a local handoff package */
    FString GenerateLocalHandoffRootPath(const FString& BlueprintName) const;

    /** Get the base path where translations are saved */
    FString GetTranslationBasePath() const;

    /** Get the base path where local handoff packages are saved */
    FString GetLocalHandoffBasePath() const;
    ...
    /** Build the handoff instructions that accompany local JSON exports */
    FString BuildLocalHandoffDocument(
    ...
    /**
     * Tracks in-flight requests independently from the UI-facing status.
     * This lets us suppress unstable UE 5.7 status-overlay transitions
     * without allowing duplicate requests.
     */
    bool bTranslationRequestInFlight = false;
};
